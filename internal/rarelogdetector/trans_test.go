...
func Test_tokenizeLine(t *testing.T) {
	...
	if cnt, cnt2, _, _, err = tr.tokenizeLine(line, 0, true, false, 0); err != nil {
		t.Errorf("%v", err)
		return
	}
	...
	if err := utils.GetGotExpErr("item count", cnt2, 1); err != nil {
		t.Errorf("%v", err)
		return
	}
	...
	// other
	tr, err = newTrans("", "", "", 0, 1000, 0, nil, nil, false, false)
	if err != nil {
		t.Errorf("%v", err)
		return
	}
